...
function TarPackage(mediaPackage, logger){
  ...
  if(!this.publishConf.timecodeFileName || (typeof this.publishConf.timecodeFileName !== "string"))
    this.emit("error", new TarPackageError("timecodeFileName in publishConf.json must be a String"), errors.INVALID_CONFIGURATION);
  
  // Validate package metadata file name
  if(!this.publishConf.metadataFileName || (typeof this.publishConf.metadataFileName !== "string"))
    this.emit("error", new TarPackageError("metadataFileName in publishConf.json must be a String"), errors.INVALID_CONFIGURATION);
...
TarPackage.prototype.preparePublicDirectory = function(){
  ...
  openVeoAPI.fileSystem.mkdir(path.normalize(process.rootPublish + "/public/publish/videos/" + this.mediaPackage.id), function(error){
...
TarPackage.prototype.saveTimecodes = function(){
  ...
  saveTimecodes(path.join(extractDirectory, this.publishConf.timecodeFileName), path.join(videoFinalDir, "synchro.json"), function(error){
    ...
    if(error)
      ...
      filesToCopy.forEach(function(file){
        ...
        openVeoAPI.fileSystem.copy(path.join(extractDirectory, file), path.join(videoFinalDir, file), function(error){
          ...
          if(error)
            self.logger.warn(error.message, {"action" : "copyImages", "mediaId" : self.mediaPackage.id});
...
TarPackage.prototype.getMediaFilePath = function(){
  return path.join(this.publishConf.videoTmpDir, "/" + this.mediaPackage.id + "/" + this.mediaPackage.metadata.filename);
};
...
function validatePackage(callback){
  ...
  openVeoAPI.fileSystem.getJSONFileContent(path.join(extractDirectory, this.publishConf.metadataFileName), function(error, packageInformation){
