 ...
 *
 * e2e task:
 * Execute OpenVeo Publish end to end tests on chrome. Plugin tests are executed within OpenVeo core.
 *
...
plan.local(['e2e', 'unit'], function(local) {
  ...
  try {
    ...
    if (local.exec('test -e ' + publishConfigurationDirectory, {failsafe: true}).code === 1) {
      if (local.exec('mkdir -p ' + publishConfigurationDirectory, {failsafe: true}).code === 1)
        local.mkdir(publishConfigurationDirectory);
    }

    // Publish configuration files if not always present
    var configurationFiles = ['publishConf.json', 'videoPlatformConf.json', 'watcherConf.json'];

    configurationFiles.forEach(function(configurationFile) {
      var destinationPath = path.join(publishConfigurationDirectory, configurationFile);
      if (local.exec('test -e ' + destinationPath, {failsafe: true}).code === 1)
    ...
    local.log('openveo-publish successfully installed');
  } catch (e) {
    plan.abort(e.message);
  }
});
...
// Execute OpenVeo Publish end to end tests using chrome
plan.local(['e2e'], function(local) {
  try {
    local.with('cd ' + workingDirectory, function() {
      local.exec(
